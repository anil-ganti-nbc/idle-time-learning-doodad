...
const DAU_PORT = 8090;
const OS_PORT = 8096;
const PL_ORIGIN = `http://localhost:${OS_PORT}`;
const LESSON_URL = `http://localhost:${DAU_PORT}/learn/os-race-10`;
...
const pl = spawn("npm", ["run", "dev"], {
  cwd: new URL("../../os-lab", import.meta.url).pathname, stdio: "ignore", detached: true,
});
...
try {
  ...
  const launchButton = host.getByRole("button", { name: "Practice in OS Lab ↗" });
  await launchButton.waitFor({ timeout: 30_000 });
  console.log("✔ DAU lesson page surfaces OS Lab");
  ...
  await lab.getByText("Practice · os-race · os-race-10").waitFor({ timeout: 20_000 });
  console.log("✔ popup launched os-lab with contract payload");
  ...
  console.log("✔ take completed inside os-lab");

  await host.getByText(/Practice logged: os-race-10/).waitFor({ timeout: 15_000 });
  ...
  const entry = logged.find((e) => e.lessonId === "os-race-10");
  if (!entry || entry.labId !== "os-lab") fail(`bad log entry: ${JSON.stringify(logged)}`);
  console.log("✔ persisted:", JSON.stringify(entry));

  await browser.close();
  console.log("\n✅ FULL-STACK DAU → OS LAB OK");
} finally {
