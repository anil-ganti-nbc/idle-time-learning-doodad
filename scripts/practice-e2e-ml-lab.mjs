...
const DAU_PORT = 8090;
const ML_PORT = 8097;
const PL_ORIGIN = `http://localhost:${ML_PORT}`;
const LESSON_URL = `http://localhost:${DAU_PORT}/learn/ml-gd-10`;
...
const pl = spawn("npm", ["run", "dev"], {
  cwd: new URL("../../ml-lab", import.meta.url).pathname, stdio: "ignore", detached: true,
});
...
try {
  ...
  const launchButton = host.getByRole("button", { name: "Practice in ML Lab ↗" });
  await launchButton.waitFor({ timeout: 30_000 });
  console.log("✔ DAU lesson page surfaces ML Lab");
  ...
  await lab.getByText("Practice · ml-gd · ml-gd-10").waitFor({ timeout: 20_000 });
  console.log("✔ popup launched ml-lab with contract payload");
  ...
  console.log("✔ take completed inside ml-lab");

  await host.getByText(/Practice logged: ml-gd-10/).waitFor({ timeout: 15_000 });
  ...
  const entry = logged.find((e) => e.lessonId === "ml-gd-10");
  if (!entry || entry.labId !== "ml-lab") fail(`bad log entry: ${JSON.stringify(logged)}`);
  console.log("✔ persisted:", JSON.stringify(entry));

  await browser.close();
  console.log("\n✅ FULL-STACK DAU → ML LAB OK");
} finally {
