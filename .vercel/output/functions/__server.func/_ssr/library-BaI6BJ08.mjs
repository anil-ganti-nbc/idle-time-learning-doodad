...
import { B as useCatalog, C as isRetiredSeededCategory, V as useProgress, w as isSelectableCategory } from "./use-catalog-DsTCgnv9.mjs";
import { t as HydrateGate } from "./hydrate-Ceh-ch8W.mjs";
import { t as conceptState } from "./state-ZIUkIptt.mjs";
import { n as SourceBadge } from "./provenance-psaqoQS3.mjs";
import { s as startLive } from "./live-DpxOBqbB.mjs";
import { t as Input } from "./input-CRYzMUF8.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/library-BaI6BJ08.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
function computeCoverage(catalog, progress = {}) {
	...
	const coursesCovered = catalog.courses.map((course) => {
		const concepts = course.modules.flatMap((m) => m.conceptIds).map((id) => catalog.conceptMap[id]).filter(Boolean);
		...
		const unseenMinutes = concepts.filter((c) => !progress[c.id]?.encountered).reduce((sum, c) => sum + (c.estimatedMinutes ?? 10), 0);
		return {
			courseId: course.id,
			title: course.title,
			...
			unseenMinutes,
			shallow: concepts.length > 0 && withLessons / concepts.length < .15
		};
	});
	const shallowModules = catalog.courses.flatMap((course) => course.modules.map((mod) => {
		const withLessons = mod.conceptIds.filter((id) => lessonConceptIds.has(id)).length;
		return {
			moduleId: mod.id,
			courseId: course.id,
			conceptCount: mod.conceptIds.length,
			withLessons
		};
	...
	const unseenMinutes = activeConcepts.filter((c) => !progress[c.id]?.encountered).reduce((sum, c) => sum + (c.estimatedMinutes ?? 10), 0);
	return {
		subjects: new Set(catalog.courses.map((c) => c.categoryId)).size,
...
function LibraryReady() {
	...
	const coverage = (0, import_react.useMemo)(() => computeCoverage(catalog, progress), [catalog, progress]);
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
				...
				children: [
					...
					" modules ·",
					" ",
					coverage.activeConcepts,
					" concepts · ~",
					hours,
					" h of skeleton. Search or open a unit — the router is still the fastest path."
				]
			}),
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("ul", {
				...
				children: catalog.categories.filter((cat) => {
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("details", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("summary", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
									...
									children: [
										cat.name,
										cat.custom ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "ml-2 text-[10px] tracking-wide text-muted uppercase",
											children: "yours"
										}) : null,
										cat.status === "retired" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "ml-2 text-[10px] tracking-wide text-muted uppercase",
											children: "archived"
										}) : null
									]
								...
								courses.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									...
									children: courses.map((course) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Link, {
										to: "/course/$courseId",
