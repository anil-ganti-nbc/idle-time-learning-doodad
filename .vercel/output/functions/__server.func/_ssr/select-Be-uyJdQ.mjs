import { M as pickCourseForLearner, O as makeReadinessContext, _ as isCategoryOpenForSelection, b as isDue, c as courseForConcept, d as frontierConcepts, s as courseForCategory, v as isConceptUnlocked, x as isLessonUnlocked, y as isDemonstrated } from "./use-catalog-DsTCgnv9.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/select-Be-uyJdQ.js
function lessonsForConcept(catalog, conceptId) {
	return catalog.lessons.filter((l) => l.conceptId === conceptId);
}
function ctxOf(catalog, progress, profile, options) {
...
function recencyPenalty(progress, now) {
	if (!progress?.lastStudiedAt) return 0;
	const hours = (now.getTime() - new Date(progress.lastStudiedAt).getTime()) / 36e5;
...
function scoreLesson(lesson, req, progress, catalog, profile, now, ctx) {
	let score = 0;
...
function pickBest(pool, req, progress, catalog, profile, options, ctx) {
...
function allowedByRetirement(lesson, catalog, req) {
	const cat = catalog.conceptMap[lesson.conceptId]?.category;
	return isCategoryOpenForSelection(catalog, cat, req.category);
}
function restrictToFrontier(pool, course, ctx) {
...
function pickSurpriseCourse(ready, recentCategories, catalog, rng, ctx) {
...
function selectLesson(req, progress, recentCategories, catalog, profile, options = {}) {
	const ctx = ctxOf(catalog, progress, profile, options);
	const fitting = catalog.lessons.filter((l) => durationFits(l, req.minutes) && inCategory(l, req.category, catalog) && notAvoided(l, catalog, profile) && allowedByRetirement(l, catalog, req));
	...
	if (req.mode === "reinforce") {
		...
		const pick = pickBest(due, req, progress, catalog, profile, options, ctx) ?? pickWeakest(seen, progress) ?? pickBest(ready, req, progress, catalog, profile, options, ctx);
	...
	if (req.mode === "surprise") {
		const targeted = req.category && req.category !== "random" ? pickCourseForLearner(catalog, req.category, ctx) : void 0;
		const bucket = targeted ? {
			course: targeted,
			categoryId: targeted.categoryId
		...
		const pick = pickBest(restrictToFrontier(unseen.length ? unseen : inBucket, bucket.course, ctx), req, progress, catalog, profile, options, ctx) ?? pickBest(ready, req, progress, catalog, profile, options, ctx);
		if (!pick) return fallbackUnready(fitting, req, progress, catalog, profile, options, ctx);
		return {
			lesson: pick,
			reason: courseForConcept(catalog, pick.conceptId) ? `A ${bucket.course ? "fresh field" : "different course"}, at the next unit you can actually hold.` : recentCategories.includes(catalog.conceptMap[pick.conceptId]?.category ?? "") ? "A concept you have not opened yet." : "Outside the topics you have been studying lately."
		};
	}
	const course = pickCourseForLearner(catalog, req.category, ctx) ?? courseForCategory(catalog, req.category);
	const inCourse = course ? ready.filter((l) => courseForConcept(catalog, l.conceptId)?.id === course.id) : ready;
	const pool = inCourse.length ? inCourse : ready;
	const unseen = pool.filter((l) => !progress[l.conceptId]?.encountered);
	const pick = pickBest(restrictToFrontier(unseen.length ? unseen : pool, course, ctx), req, progress, catalog, profile, options, ctx);
	if (!pick) return fallbackUnready(fitting, req, progress, catalog, profile, options, ctx);
	return {
		lesson: pick,
		reason: course ? unseen.length ? "Next unit in this course whose prerequisites you already hold." : "You have seen the open units that fit. Revisiting the best match." : unseen.length ? "A new concept whose prerequisites you already hold." : "You have seen the new units that fit. Revisiting the best match."
	};
}
...
function fallbackUnready(fitting, req, progress, catalog, profile, options, ctx) {
	...
	const missing = (concept?.prerequisites ?? blocked.prerequisites).filter((id) => !isDemonstrated(id, ctx, inferTierSafe(concept ?? {
		id: "",
		name: "",
		category: "",
		prerequisites: [],
	...
	const foundation = pickBest(missing.flatMap((id) => lessonsForConcept(catalog, id).filter((l) => durationFits(l, req.minutes) && isLessonUnlocked(l, ctx))), req, progress, catalog, profile, options, ctx);
	if (!foundation) {
...
function scoreMissingConcept(concept, req, progress, recentCategories, catalog, profile, options = {}) {
	const ctx = ctxOf(catalog, progress, profile, options);
	let score = 0;
	...
	const parent = concept.parentId ? progress[concept.parentId] : void 0;
	if (parent?.understanding === "didnt_get_it" || parent && (parent.lastQuizScore ?? 1) < .67) score += 4;
	...
	const siblings = catalog.concepts.filter((c) => c.parentId === concept.parentId && c.category === concept.category && c.id !== concept.id);
...
function missingConceptForGeneration(req, progress, recentCategories, catalog, profile, options = {}) {
	const ctx = ctxOf(catalog, progress, profile, options);
	...
	const pick = pickFromScored(withoutFit.map((concept) => ({
		item: concept,
...
function nextConcepts(catalog, progress, _journalist, knownIds, courses) {
